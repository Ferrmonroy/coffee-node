...
describe("COFFEE CRUD", () => {
  it("200 STATUS CODE, BODY IS AN ARRAY WITH AT LEAST 1 OBJECT.", async () => {
  ...
  it("404 STATUS CODE AFTER DELETING A NON-EXISTENT ID", async () => {
    const jwt = "token";
  ...
  it("400 STATUS CODE AFTER TRYING UPDATING A COFFEE WITH A DIFFERENT/NON-EXISTING ID", async () => {
    const { body } = await request(server).get("/cafes").send();
    const newID = creatingNewId(body);
